...
for h to hypotheses
     ...
     selectObject: hypothesis
      ...
      for r to Object_'hypothesis'.nrow
          ...
          selectObject: string_object
          line_orig$ = Get string: c + count
          line_orig$ = replace_regex$(line_orig$, "[0-9][0-9]* [0-9][0-9]*", "", 0)
          line$ = replace_regex$(line$, "[^0-9 ]", "", 0)
          pard_line$ = line$ + " " + line_orig$
          pard_line$ = replace_regex$(pard_line$, "   ", " ", 0)
          pard_line$ = replace_regex$(pard_line$, "  ", " ", 0)
          pard_line$ = replace_regex$(pard_line$, "  ", " ", 0)
          appendFileLine: output_file$, pard_line$
          count = count + 1
          selectObject: hypothesis
      endfor
